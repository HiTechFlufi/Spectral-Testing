...
export class User extends Chat.MessageContext {
	...
	handleRename(name: string, userid: ID, newlyRegistered: boolean, userType: string) {
		...
		if (userType !== '1') {
			...
			if (userType === '3') {
				...
				Punishments.ban(this, Date.now() + PERMALOCK_CACHE_TIME, userid, true, `Permabanned as ${name}`);
			}
		}
	...
	async tryJoinRoom(roomid: RoomID | Room, connection: Connection) {
		...
		if (room.ttBattle) {
			// @ts-ignore
			let ttRoom = Server.getTeamTourRoom(room.p1.id);
			let players = [];
			// @ts-ignore
			if (Server.findTeamMatchUp(room.p1.id, room.p2.id) && Rooms.get(ttRoom) && Rooms.get(ttRoom).teamTours) {
				// @ts-ignore
