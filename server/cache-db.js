...
class CacheDB {
	...
	load(dir) {
		...
		if (!FS(dir).readIfExistsSync()) FS(dir).writeSync("{}");
		try {
			this.cache = JSON.parse(FS(dir).readSync());
		} catch (e) {
	...
	write() {
		...
		if (this.dir && this.changes) {
			...
			FS(this.temp).write(JSON.stringify(this.cache));
			// if success, rename the file.
			FS(this.temp).rename(this.dir);
		}
	}
	...
	drop() {
		...
		// drop the file
		if (dir) FS(dir).unlinkIfExistsSync();
	}
