...
const commands = {
	...
	roomdesc(target, room, user) {
		...
		if (!this.can('declare') && room.founder !== user.id) return false;
	...
	roomowner(target, room, user) {
		...
		if (!this.can('makeroom') && user.id !== room.founder) return false;
		...
		if (targetUser) {
			targetUser.popup(`|html|You were appointed Room Owner by ${Server.nameColor(user.name)} in ${room.roomid}.`);
	...
	roompromote(target, room, user, connection, cmd) {
		...
		if (this.pmTarget && targetUser) {
			...
			if (needsPopup) targetUser.popup(`|html|You were demoted to Room ${groupName} by ${Server.nameColor(user.name)} in ${room.roomid}.`);
		} else if (nextGroup === '#') {
			this.addModAction(`${'' + name} was promoted to ${groupName} by ${user.name}.`);
			this.modlog('ROOM OWNER', userid);
			if (needsPopup) targetUser.popup(`|html|You were promoted to ${groupName} by ${Server.nameColor(user.name)} in ${room.roomid}.`);
		} else {
			this.addModAction(`${'' + name} was promoted to Room ${groupName} by ${user.name}.`);
			this.modlog(`ROOM${groupName.toUpperCase()}`, userid);
			if (needsPopup) targetUser.popup(`|html|You were promoted to Room ${groupName} by ${Server.nameColor(user.name)} in ${room.roomid}.`);
		}
	...
	async hotpatch(target, room, user, connection, cmd) {
		...
		if (!this.can('hotpatch') && !Server.isDev(user.id)) return;
	...
	lockdown(target, room, user) {
		if (!this.can('lockdown') && !Server.isDev(user.id)) return false;
	...
	async updateserver(target, room, user, connection) {
		if (!user.can('hotpatch') && !Server.isDev(user.id)) {
