...
function buildMenu(userid, num, tier) {
	if (!Server.TB[userid]) return '<span style="color:red"><b>Error: </b>User "' + userid + '" not found in TeamBuilder.</span>';
	num = parseInt(num);
	let speciesName = toID(Server.TB[userid].monArray[tier][num].species);
	let split = Server.TB[userid].monArray[tier][num].species.split('-');
	if (split.length > 1) {
		speciesName = toID(split[0]) + '-' + speciesName.substring(toID(split[0]).length);
	}
	let output = '';
	for (let u = 0; u < Server.TB[userid].monArray[tier].length; u++) {
		output += '<button name="send" value="/teambuilder edit ' + tier + ', ' + (u + 1) + '"><img src="//play.pokemonshowdown.com/sprites/bw/' + toID(Server.TB[userid].monArray[tier][u].species) + '.png" height="32" width="32"></button>&nbsp;';
	}
	if (Server.TB[userid].monArray[tier].length !== 6) output += '<button name="send" value="/teambuilder addmon ' + tier + ', ' + Object.keys(tierDexList[tier])[0] + '"><i class="fa fa-plus"></i></button>';
	output += '<button name="send" class="button" value="/teambuilder export ' + tier + '">Export Team</button><button name="send" class="button" value="/teambuilder removemon ' + tier + ', ' + (num + 1) + '"><i class="fa fa-trash"></i>Delete</button><button name="send" class="button" value="/dt ' + toID(Server.TB[userid].monArray[tier][num].species) + ', ' + tier + '">Mon data</button><div class="setchart" style="height: 155px;">';
	output += '<div class="setcol setcol-icon"><div class="setcell-sprite"><img src="//play.pokemonshowdown.com/sprites/bw/' + speciesName + '.png" height=80 width=80></div><div class="setcell setcell-pokemon"><label>Pokémon</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/teambuilder monlist ' + tier + '">' + Server.TB[userid].monArray[tier][num].species + '</button></div></div>';
	output += '<div class="setcol setcol-details"><div class="setrow"><div class="setcell setcell-details"><label>Details</label><button class="textbox setdetails" tabindex="-1" name="send" value="/teambuilder details main ' + tier + ', ' + (num + 1) + '"><span class="detailcell detailcell-first"><label>Level</label>' + Server.TB[userid].monArray[tier][num].level + '</span><span class="detailcell"><label>Gender</label>' + (Server.TB[userid].monArray[tier][num].gender === 'random' ? '-' : Server.TB[userid].monArray[tier][num].gender) + '</span><span class="detailcell"><label>Happiness</label>' + Server.TB[userid].monArray[tier][num].happiness + '</span><span class="detailcell"><label>Shiny</label>' + (Server.TB[userid].monArray[tier][num].shiny ? 'Yes' : 'No') + '</span></button><span class="itemicon" style="background: none"></span></div></div><div class="setrow"><div class="setcell setcell-item"><label>Item</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/teambuilder itemlist ' + tier + ', ' + (num + 1) + '">' + (Server.TB[userid].monArray[tier][num].item ? Server.TB[userid].monArray[tier][num].item : '') + '</button></div><div class="setcell setcell-ability"><label>Ability</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/teambuilder ability ' + tier + ', blank, ' + (num + 1) + '">' + Server.TB[userid].monArray[tier][num].ability + '</button></div></div></div>';
	output += '<div class="setcol setcol-moves"><div class="setcell"><label>Moves</label><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left;" name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Server.TB[userid].monArray[tier][num].moves[0] ? Server.TB[userid].monArray[tier][num].moves[0] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left;" name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Server.TB[userid].monArray[tier][num].moves[1] ? Server.TB[userid].monArray[tier][num].moves[1] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left;" name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Server.TB[userid].monArray[tier][num].moves[2] ? Server.TB[userid].monArray[tier][num].moves[2] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; " name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Server.TB[userid].monArray[tier][num].moves[3] ? Server.TB[userid].monArray[tier][num].moves[3] : '') + '</button></div></div>';
	...
	for (let i = 0; i < statNames.length; i++) {
		stats[toID(statNames[i])] = getStat(toID(statNames[i]), {
			species: Server.TB[userid].monArray[tier][num].species,
			evs: Server.TB[userid].monArray[tier][num].evs,
			ivs: Server.TB[userid].monArray[tier][num].ivs,
			nature: Server.TB[userid].monArray[tier][num].nature,
			level: Server.TB[userid].monArray[tier][num].level,
		});
		let evBuf = '<em>' + (Server.TB[userid].monArray[tier][num].evs[toID(statNames[i])] === 0 ? '' : Server.TB[userid].monArray[tier][num].evs[toID(statNames[i])]) + '</em>';
		if (Dex.mod(tier).getNature(Server.TB[userid].monArray[tier][num].nature).plus === toID(statNames[i])) {
			evBuf += '<small>+</small>';
		} else if (Dex.mod(tier).getNature(Server.TB[userid].monArray[tier][num].nature).minus === toID(statNames[i])) {
			evBuf += '<small>&minus;</small>';
		}
		let width = stats[toID(statNames[i])] * 75 / 504;
		if (statNames[i] === 'HP') width = stats[toID(statNames[i])] * 75 / 704;
		if (width > 75) width = 75;
		let color = Math.floor(Server.TB[userid].monArray[tier][num].evs[toID(statNames[i])] * 180 / 714);
...
function moveMenu(userid, num, tier) {
	num = parseInt(num);
	let output = '';
	output += '<div class="setchart" style="text-align:center; height: 150px;"><h3><u>Move Menu</u><br /><button name="send" class="button" value="/dt ' + toID(Server.TB[userid].monArray[tier][num].species) + ',' + tier + '">Mon data</button></h3><div style="padding-bottom: 2px">To remove a move click the button with the move name<br /><i>Current Moves:</i> ';
	for (let i = 0; i < Server.TB[userid].monArray[tier][num].moves.length; i++) {
		if (Server.TB[userid].monArray[tier][num].moves.length === 0) break;
		output += '<button name="send" class="button" value="/teambuilder removemove ' + tier + ', ' + Server.TB[userid].monArray[tier][num].moves[i] + ', ' + (num + 1) + '">' + Server.TB[userid].monArray[tier][num].moves[i] + '</button> , ';
	}
...
function abilityMenu(userid, num, tier) {
	num = parseInt(num);
	let output = '<div class="setchart" style="text-align:center"><h3><u>Ability Menu</u></h3><div style="padding-bottom: 2px"><i>Current Ability:</i> ' + Server.TB[userid].monArray[tier][num].ability + '</div><div style="padding-bottom: 2px"></div>';
	let pokemon = Dex.mod(tier).getTemplate(Server.TB[userid].monArray[tier][num].species);
...
class TeamBuilder {
	...
	species(mon, monSlot, tier) {
		...
		for (let p in dex) {
			if (dex[p].num || dex[p].species === 'Missingno.') continue;
			let pokemon = dex[p];
	...
	getMonArray(slot, userid, tier) {
		if (!Server.TB[userid].monArray[tier][slot - 1]) return "That slot does not have a mon!";
		return slot - 1;
	}
}
...
try {
	...
	for (let first in raw) {
		for (let u in raw[first].monArray) {
			Server.TB[first] = new TeamBuilder(first, u);
		}
		for (let second in Server.TB[first]) {
			Server.TB[first][second] = raw[first][second];
		}
	}
} catch (e) {
...
exports.commands = {
	...
	teambuilder: {
		...
		edit: function (target, room, user) {
			...
			if (!Server.TB[user.id].monArray[tier]) {
				...
				Server.TB[user.id].monArray[tier] = [{
					gender: 'random',
					level: 100,
					happiness: 255,
					item: false,
			...
			if (slot === 0) ++slot;
			let num = Server.TB[user.id].getMonArray(slot, user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			Server.TB[user.id].lastMonNum = num;
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, num, tier)}`);
		},

		monlist: function (target, room, user) {
			if (!user.named) return this.errorReply('You need a name!');
			...
			for (let u in dex) {
				if (dex[u] && dex[u].species && dex[u].species !== 'Missingno.' && !dex[u].num && !dex[u].num && mons.indexOf(dex[u].species) === -1) mons.push(dex[u].species + ': <button name="send" class="button" value="/dt ' + toID(dex[u].species) + ', ' + tier + '">Mon data</button>&nbsp;<button name="send" class="button" value="/teambuilder addmon ' + tier + ', ' + toID(dex[u].species) + '">Add to teambuilder as new mon</button>&nbsp;<button name="send" class="button" value="/teambuilder species ' + tier + ', ' + toID(dex[u].species) + ', ' + (Server.TB[user.id].lastMonNum + 1) + '">Replace current mon slot</button>');
			}
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, Server.TB[user.id].lastMonNum, tier)}<br />${tier[0].toUpperCase() + tier.slice(1)} Pokemon list: <br />${mons.sort().join('<br />')}`);
		},

		abilityinfo: function (target, room, user) {
			...
			if (!tiersList.includes(tier)) return this.errorReply(`${toID(tier)} is not a custom teambuilder tier. Available tiers: ${tiersList.join(',')}`);
			this.parse(`/dt ${ability}, ${tier}`);
		},

		itemlist: function (target, room, user) {
			if (!user.named) return this.errorReply(`You need a name!`);
			let [tier, slot] = target.split(',').map(p => {
				return p.trim();
			});
			...
			let itemList = [];
			let items = require('../../data/mods/' + tier + '/items').BattleItems;
			for (let u in items) {
				if (!items[u].gen && !items[u].num) itemList.push('Item: ' + Dex.mod(tier).getItem(u).name + '; Description: ' + Dex.mod(tier).getItem(u).desc + (slot >= 0 ? '<button name="send" class="button" value="/teambuilder item ' + tier + ', ' + u + ', ' + parseInt(slot) + '">Add item to mon slot ' + parseInt(slot) + '</button>' : ''));
			}
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, Server.TB[user.id].lastMonNum, tier)}<br />${tier[0].toUpperCase() + tier.slice(1)} Item list: <br />${itemList.sort().join('<br />')}`);
		},

		addmon: function (target, room, user, connection, cmd) {
			...
			// exclude dex numbers for this to work.
			for (let u in dex) {
				if (dex[u] && dex[u].species && dex[u].species !== 'Missingno.' && !dex[u].num && !dex[u].num && mons.indexOf(dex[u].species) === -1) mons.push(toID(dex[u].species));
			}
			...
			if (Server.TB[user.id].monArray[tier].length === 6) return this.errorReply(`You already have 6 pokemon!`);
			Server.TB[user.id].monArray[tier][Server.TB[user.id].monArray[tier].length] = {
				gender: 'random',
				level: 100,
				happiness: 255,
				item: false,
			...
			Server.TB[user.id].lastMonNum = Server.TB[user.id].monArray[tier].length - 1;
		    writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, Server.TB[user.id].monArray[tier].length - 1, tier)}`);
		},
		...
		species: function (target, room, user) {
			if (!user.named) return this.errorReply(`You need a name!`);
			...
			let num = Server.TB[user.id].getMonArray(parseInt(slot), user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			Server.TB[user.id].species(pkmn, num, tier);
			Server.TB[user.id].lastMonNum = num;
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, num, tier)}`);
		},
		...
		removemon: function (target, room, user) {
			if (!user.named) return this.errorReply(`You need a name!`);
			let [tier, slot] = target.split(',').map(p => {
				return p.trim();
			});
			...
			let num = Server.TB[user.id].getMonArray(slot, user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			if (Server.TB[user.id].monArray[tier].length === 1) return this.errorReply(`You need more than 1 pokemon to remove a pokemon!`);
			Server.TB[user.id].monArray[tier].splice(num, 1);
			let next = num - 1;
			if (num === 0 && Server.TB[user.id].monArray[tier].length !== 1) next = num + 1;
			if (num === 0 && Server.TB[user.id].monArray[tier].length === 1) next = num;
			Server.TB[user.id].lastMonNum = next;
			writeTB();
		...
		stats: {
			...
			iv: function (target, room, user) {
				...
				let num = Server.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Server.TB[user.id].ivs(stat, ivs, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${statMenu(user.id, num, tier)}`);
			},
			...
			nature: function (target, room, user) {
				...
				let num = Server.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Server.TB[user.id].nature(nature, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${statMenu(user.id, num, tier)}`);
			},
		...
		ability: function (target, room, user) {
			if (!user.named) return this.errorReply(`You need a name!`);
			...
			if (!Server.TB[user.id]) {
				slot = 0;
				this.sendReply(`ould not find your teambuilder pokemon, creating a new one...`);
				Server.TB[user.id] = new TeamBuilder(user.id, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, 0, tier)}`);
			}
			...
			Server.TB[user.id].ability(ability, num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${abilityMenu(user.id, num, tier)}`);
		},
		...
		move: function (target, room, user) {
			...
			if (isNaN(num)) return this.errorReply(num);
			let display = '';
			let learnset = Dex.mod(tier).getTemplate(toID(Server.TB[user.id].monArray[tier][num].species)).learnset;
			if (!learnset) return this.errorReply(`${toID(Server.TB[user.id].monArray[tier][num].species)} does not seem to have a learnset. Make sure there is a learnset file for this format and includes every mon.`);
			learnset = Object.keys(learnset).sort();
			for (let u of learnset) {
				display += 'Move: ' + Dex.mod(tier).getMove(u).name + ' Description: ' + Dex.mod(tier).getMove(u).desc + ' <button name="send" class="button" value="/teambuilder move ' + tier + ', ' + Dex.mod(tier).getMove(u).id + ', ' + (num + 1) + '">Add to ' + Server.TB[user.id].monArray[tier][num].species + '</button><br /><br />';
			}
			if (toID(move) === 'blank') return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${moveMenu(user.id, num, tier)}<br />${display}`);
			Server.TB[user.id].addMove(move, this, num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${moveMenu(user.id, num, tier)}<br />${display}`);
		},
		movehelp: [`/teambuilder move (tier), (move), (mon slot) - sets a move from the mon's repesctive tier. Mon slot is 1-6`],

		removemove: function (target, room, user) {
			if (!user.named) return this.errorReply(`You need a name!`);
			let [tier, move, slot] = target.split(',').map(p => {
				return p.trim();
			});
			...
			Server.TB[user.id].removeMove(toID(move), num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${moveMenu(user.id, num, tier)}<br />${display}`);
		},
		...
		item: function (target, room, user) {
			...
			let num = Server.TB[user.id].getMonArray(slot, user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			Server.TB[user.id].item(toID(item), num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, num, tier)}`);
		},
		...
		details: {
			...
			gender: function (target, room, user) {
				...
				let num = Server.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Server.TB[user.id].gender(gender, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${detailMenu(user.id, num, tier)}`);
			},
			...
			happiness: function (target, room, user) {
				...
				let num = Server.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Server.TB[user.id].happiness(hap, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${detailMenu(user.id, num, tier)}`);
			},
			...
			/*nickname: function (target, room, user) {
				if (!user.named) return this.errorReply(`You need a name!`);
				if (!target) return this.parse(`/help teambuilder details nickname`);
				let [tier, name, monSlot] = target.split(',').map(p => {
					return p.trim();
				});
				let tier = toID(tier);
				if (!tiersList.includes(tier)) return this.errorReply(`${toID(tier)} is not a custom teambuilder tier. Available tiers: ${tiersList.join(',')}`);
				if (!name) return this.parse(`/help teambuilder details nickname`);
				if (!monSlot) monSlot = 0;
				if (!Server.TB[user.id]) {
					this.sendReply(`Could not find your teambuilder pokemon, creating a new one...`);
					Server.TB[user.id] = new TeamBuilder(user.id, tier);
					writeTB();
					return user.sendTo(room, '|uhtml|teambuilder' + user.id + '|' + buildMenu(user.id, 0, tier));
				}
				if (monSlot === 0) monSlot = monSlot + 1;
				let num = Server.TB[user.id].getMonArray(monSlot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Server.TB[user.id].nickname(targets[0], num, tier);
				writeTB();
				return user.sendTo(room, '|uhtml|teambuilder' + user.id + '|' + buildMenu(user.id, num, tier));
			},
			nicknamehelp: [`/teambuilder details nickname (tier), (name), (mon slot) - sets a nick name for the selected mon. Mon slot is 1-6`],*/
		},
		export: function (target, room, user) {
			...
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuiler\n|pagehtml|${buildMenu(user.id, Server.TB[user.id].lastMonNum, tier)}<br /><strong>Teambuilder team</strong>:<br/><br/><div style="width: 500px; overflow-x: scroll;" class="textbox chartinput">${Dex.packTeam(Server.TB[user.id].monArray[tier])}</div><br/><br/>Paste the above into the import team section in teambuilder.`);
		},
		'': 'help',
	...
	teambuilderhelp: [
		`|raw|Custom Meta Teambuilder Help Commands:
