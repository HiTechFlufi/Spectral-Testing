	...
	splatoon: {
		...
		splatfest: {
			...
			on(target, room, user) {
				...
				if (Rooms.get("splatoon")) {
					Rooms.get("splatoon").addRaw(`${Server.nameColor(user.name, true)} has enabled Splatfest. The teams of this Splatfest are: ${team1} and ${team2}.`);
				}
				// Set Splatfest Data
			...
			off(target, room, user) {
				...
				if (Rooms.get("splatoon")) {
					Rooms.get("splatoon").addRaw(`${Server.nameColor(user.name, true)} has disabled Splatfest.`);
				}
			},
