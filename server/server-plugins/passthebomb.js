...
class PassTheBomb {
	...
	updateJoins() {
		...
		if (this.players.size > 0) {
			msg += `<center><strong>${this.players.size}</strong> ${(this.players.size === 1 ? `user has` : `Users.get( have`)} joined: ${Chat.toListString(this.players).map(player => Server.nameColor(player[1].name))}</center>`;
		}
