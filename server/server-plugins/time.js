...
exports.commands = {
	...
	ontime(target, room, user) {
		...
		if (isConnected && !Ontime[userid] && userid === isConnected) Ontime[userid] = Date.now();

		if (isConnected) {
			this.sendReplyBox(`${Server.nameColor(userid, true, true)}'s total on time on the server is <strong>${displayTime(convertTime(totalOntime))}</strong>. Current ontime: <strong>${displayTime(convertTime((currentOntime)))}</strong>.`);
		} else {
			this.sendReplyBox(`${Server.nameColor(userid, true, true)}'s total on time on the server is <strong>${displayTime(convertTime(totalOntime))}</strong>. Currently not online.`);
		}
	},
