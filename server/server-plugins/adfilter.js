...
let adWhitelist = [toID(Config.serverName)];
...
Config.chatfilter = function (message, user, room, connection, targetUser) {
	...
	if (!user.can("lock") && (pre_matches >= 1 && final_check === 0 || pre_matches >= 2 && final_check >= 1 || message.match(adRegex))) {
		...
		if (user.advWarns > 1) {
			...
			Monitor.log(`[AdMonitor] ${user.name} has been locked for attempting to advertise ${(room ? `. **Room:** ${room.roomid}` : ` in a private message to ${targetUser.name}.`)} **Message:** ${message}`);
			return false;
		}
		Monitor.log(`[AdMonitor] ${user.name} has attempted to advertise ${(room ? `. **Room:** ${room.roomid}` : ` in a private message to ${targetUser.name}.`)} **Message:** ${message}`);
