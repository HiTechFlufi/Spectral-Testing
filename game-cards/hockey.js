	...
	buildScreen(screen, options = {}) {
		...
		case "search":
			...
			for (let id in HOCKEY_GAMES) {
				...
				html += `<tr><td>${Server.nameColor(game.host.name, true, true)}</td><td${game.players.length >= game.options.cap ? ` style="color: red"` : ``}>${game.players.length}/${game.options.cap}</td><td><strong style="color: ${names[game.options.names[0]].color}">${names[game.options.names[0]].name}</strong> VS <strong style="color: ${names[game.options.names[1]].color}">${names[game.options.names[1]].name}</strong></td><td>${game.options.randomTeams}</td><td>${game.options.pin ? `<i class="fa fa-lock"></i>` : `<i class="fa fa-unlock"></i>`}</td><td><button class="button${game.players.length >= game.options.cap || game.phase !== "signups" ? ` disabled"` : `" name="send" value="/hockey join ${id}"`}>Join Game</button><button class="button" name="send" value="/hockey spectate ${id}">Spectate</button></td></tr>`;
			}
			html += `</tbody></table>`;
		...
		case "setTeams":
			...
			if (!isHost) html += `<center><p><strong style="color: #FFF">Please wait while ${Server.nameColor(game.host.name, true, true)} assigns teams.</strong></p></center>`;
		...
		case "rink":
			...
			if (game.puck.holder && game.puck.holder !== this.userid) html += `<center><strong style="color: #FFF">${Server.nameColor(game.puck.holder, true, true)}, from <strong style="color: ${names[Users.get(game.puck.holder).console.team].color}">Team ${names[Users.get(game.puck.holder).console.team].name}</strong>, has the puck! (ZONE: ${puckZone})</strong></center><br />`;
...
class HockeyGame {
	...
	updateAll(screen, options = {}) {
		for (let i = 0; i < this.players.length; i++) {
			let u = Users.get(this.players[i]);
		...
		for (let i = 0; i < this.spectators.length; i++) {
			let u = Users.get(this.spectators[i]);
	...
	confirmTeams() {
		for (let i = 0; i < this.players.length; i++) {
			let player = Users.get(this.players[i]);
